...
public class ShowCharacterInventoryCommand : CommandBase
{
    protected override Regex CommandPattern { get; set; } = new(@"^инвентарь\s*$", RegexOptions.IgnoreCase);
    public override async void Execute(Chat chat, User user)
    {
        if(!CharacterManager.TryGetCharacter(chat, user, out var character))
            return;

        await Bot.SendTextMessageAsync(character.InventoryInfo());
    }
}